...
def main():
    ...
    train_filedir = file_prefix + '/benign'
    detect_filedir = file_prefix + '/attack'
    ...
    save_data_to_npz(attack_train_X, attack_train_Y, file_prefix + '/attack_split/attack_train.npz')
    save_data_to_npz(attack_test_X, attack_test_Y, file_prefix + '/attack_split/attack_test.npz')


    train_files = find_npz_files(train_filedir)
    if(NEW_CLASS_TEST == 1):
        print("new class comes out===============")
        #new_class_dir = 'new_class'
        #if not os.path.exists(new_class_dir):
            #os.makedirs(new_class_dir)
        if len(train_files) < 2:
            print("the number of file is too few")
        else:
        ...
        #new_benign_label = np.array(new_benign_label)
        #print(new_benign_label)
        ...
        for label, count in zip(unique_labels, label_counts):
            if(count > 100):
                print(f"Class {label} sample number is：{count}, it's ok")
            else:
                print(f"Class {label} sample number is：{count}, not ok, warting")
        ...
        for category in new_categories:
            ...
            #new_test_data[category] = {
            #    'normalized_VAE_test_X': new_normalized_VAE_test_X,
            #    'VAE_test_Y': new_VAE_test_Y
            #}
    ...
    save_data_to_npz(benign_train_X, benign_train_Y, file_prefix + '/benign_split/benign_train.npz')
    save_data_to_npz(benign_test_X, benign_test_Y, file_prefix + '/benign_split/benign_test.npz')
    
    

    train_benign_file = np.load(file_prefix + '/benign_split/benign_train.npz')
    test_benign_file = np.load(file_prefix + '/benign_split/benign_test.npz')
    train_attack_file = np.load(file_prefix + '/attack_split/attack_train.npz')
    test_attack_file = np.load(file_prefix + '/attack_split/attack_test.npz')
    ...
    original_test_attack_X = test_attack_file['X']#
    original_test_attack_Y = test_attack_file['Y']#

    train_attack_X, test_attack_X = normalize_data(train_attack_X,test_attack_X)
    ...
    for category in categories:
        ...
        original_matched_attack_X, original_matched_attack_Y = match_and_select_data(category, original_test_attack_X, original_test_attack_Y)#modified on 20250520
        normalized_VAE_train_X2, normalized_VAE_test_attack_X = normalize_data(VAE_train_X, original_matched_attack_X)

        test_data[category] = {
            'normalized_VAE_test_X': normalized_VAE_test_X,
            'VAE_test_Y': VAE_test_Y,
            'normalized_VAE_test_attack_X':normalized_VAE_test_attack_X,
            'VAE_test_attack_Y':original_matched_attack_Y
        }
    ...
    with open(os.path.join(file_prefix, 'test_data.pkl'), 'wb') as f:
    ...
    #model_fsl.load_state_dict(torch.load('resnet_siamese_model.pth'))
    ...
    for filename in os.listdir(model_dir):
        #if filename.startswith('model_') and filename.endswith('192.168.10.50_21_6.pth'):
    ...
    pkl_file_path = file_prefix + '/test_data.pkl'
    ...
    for category, data in test_data.items():
        #if(category != '192.168.10.50_21_6'):
        #    continue
        ...
        matched_attack_X = data['normalized_VAE_test_attack_X']
        matched_attack_Y = data['VAE_test_attack_Y']
        ...
        #true_labels = results_df['Labels'].values
    ...
    if(len(indices) == 0):
        ...
        accuracy, similarities = evaluate_model(fsl_model_load, test_loader, X_train,y_train,class_means, class_to_label, parameters['unknown_class'], plot=False)
...
def split_unknown_by_class(all_X, all_Y, num_samples_per_class=None, unknown_class = 2):
    ...
    for cls in classes:
        ...
        if cls == 'benign' or '_6' in cls or '_17' in cls:
            ...
            train_Y.append(np.hstack((class_Y[:split_point, :1], new_labels)))  # 合并原始类别和新标签
...
def split_data_by_class(all_X, all_Y, split_type='ratio', split_value=0.8, num_samples_per_class=None):
    ...
    current_label = 1
        ...
        #print(len(all_X))
        #print(len(all_Y))
        #raise ValueError("all_X and all_Y must have the same length")
    all_X = np.array(all_X)
    all_Y = np.array(all_Y)
    for cls in classes:
        class_mask = all_Y[:, 0] == cls
